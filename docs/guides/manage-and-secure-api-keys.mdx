...
If you work in the technology space you know APIs are a valuable part of building great products. APIs are a gateway to explore services, integrations and send-receive data from remote servers which helps to build great products faster.

APIs have been proven as an efficient and time-saving solution, so it's important to understand how to securely access and use them. Most APIs require some form of authentication in order to send & receive data.

You can read more about the API authentication and authorization <Link to="/quickstart/api-authentication">**HERE**</Link>

## Managing your DataHub API Keys

To access Figment's APIs, you must authenticate your requests using a valid API key. You can create, view and delete your API keys from the DataHub dashboard. You'll find more information below about securing your API keys.
...
2. Select the App from the Project list you wish to create a new API key for.
...
Note: You must have at least 2 API keys before you can delete the original API key.

1. Log in to your account.
2. Select the App from the Project list you wish to delete the API key from.
3. Click on the Overview tab of the dashboard.
4. Click on View Key.
...
5. **Never share your secrets unencrypted**, in emails or via messaging systems like Slack or Discord. Always use a secrets manager, vault or password manager such as [AWS Key Management Service](https://aws.amazon.com/kms/), [Hashicorp Vault](https://www.vaultproject.io/), [1Password](https://1password.com/developers/) or similar.
