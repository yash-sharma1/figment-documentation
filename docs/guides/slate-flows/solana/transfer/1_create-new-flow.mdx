...
4. `amount` (**required**) — The amount of SOL to be transferred to the account. This field should be in full SOL, not in lamports (ie. if you want to transfer 5 SOL, return `"amount": 5.0`). Slate will translate the amount to lamports so you do not have to worry about the conversion factor.
