---
title: Working With Slate Flows
sidebar_position: 1
---
...
Slate has two endpoints for starting and working through a Flow:
...
![Slate Assign Staking Account](/img/staking/slate_assign_stake_account.png)
...
- `sign_[transaction_type]_tx`: you will return a signed transaction payload or signing payload to Slate in order for Slate to broadcast it to the network
...
After you submit data during a data collection step, the response returned by Slate will look similar to this (NEAR token transfer Flow used for this example):
...
Slate will expect to receive EITHER a signed transaction payload OR an array of signatures obtained from signing the `signing_payload`.

If you provide signatures, then Slate will construct the signed transaction payload for you.
...
We encourage you to explore the code in order to understand how it works. You can run the code locally on your own system. It does not connect to Slate endpoints in any way, also you do not provide any identifying information for the transaction.
...
Similarly, the NPM package will return a signing payload - this should exactly match the one you received in the response from Slate. In this way you can be assured that the payload you sign belongs to the same transaction.

### Signing the Transaction

If you are using Fireblocks as a custodial solution check the following page for instructions on using the raw signing capabilities of Fireblocks: <Link to="/guides/slate-flows/fireblocks-signing-transactions">Signing Slate Transactions with Fireblocks</Link>
