...
It’s also known as a "majority attack." When a miner or group of miners controls more than 50% of the mining power of the network, they can disrupt the network’s mining process and manipulate the network.
...
The process of distributing digital assets to the public—just like transferring some tokens to users' wallet addresses. Often used as a marketing strategy to promote awareness of a new token.
...
An archive node stores everything kept in the full node and also builds an archive of historical states. They are necessary if you want to check the state of an account at any given block height.
...
All-time high (ATH) is the highest value/price of a cryptocurrency ever reached in history.
...
Inspired or derived from HODL (an acronym for "hold on for dear life"), a common cryptocurrency investment strategy. BUIDL refers to keeping your head down and focusing on building your product.
...
A digital currency that is built using blockchain technology and secured by cryptography to use as a medium of peer-to-peer transaction. such as BTC, ETH, and others.
...
The EigenTrust Algorithm decreases the number of inauthentic files on a network, even under a variety of conditions where malicious peers cooperate in an attempt to deliberately subvert the system.
...
The Ethereum Virtual Machine. A runtime environment used by smart contracts on some Web3 protocols including Ethereum.
...
This is an acronym for "hold on for dear life" in the crypto industry. Investors buy and hold digital assets indefinitely.
...
It refers to the maximum number of coins/tokens that can be ever created for a specific cryptocurrency. For example, it’s 21 million for Bitcoin.
...
A hash-based data structure used to organize large amounts of data.
...
**Open web (not OpenWeb)**

The public side of the Web. Not to be confused with OpenWeb, which is a private company.
...
Plasma is a framework for improving scalability in public blockchains. Plasma chains are built atop another blockchain.
...
Representational state transfer (REST). With REST, the server does not store any information about the client; every request from the client contains all the information for the server to process the request.
...
Remote procedure call (RPC) allows a program on one machine to call a subroutine on another machine without knowing that it is remote.
...
Three-tier architecture has three tiers that each can be developed simultaneously by a separate development team, and can be updated or scaled as needed without impacting the other tiers.
