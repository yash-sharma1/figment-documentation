...
After creating the new staking flow, we need to submit the staking account data to create a new staking account and move forward. After collecting the required inputs as mentioned below, send a PUT request to `/api/v1/flows/[:flow_id]/next` to proceed to the next step.
If you already have a staking account, proceed to the step <Link to="/guides/staking-api/Solana/delegate/submit-staking-account-data">Submit Staking Account Data</Link>
