...
| `polkadot`    | `mainnet`, `westend`       | `add_proxy`, `remove_proxy`, `staking`, `unstaking`, `transfer` |
| `solana`      | `mainnet`, `devnet`        | `staking`, `unstaking`, `transfer`                              |
...
If you are using Fireblocks as a custodial solution, check the following page for instructions on using the raw signing capabilities of Fireblocks: <Link to="/guides/staking-api/fireblocks-signing-transactions">Signing Transactions with Fireblocks</Link>
