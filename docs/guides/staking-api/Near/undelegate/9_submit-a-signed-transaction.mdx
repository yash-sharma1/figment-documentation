---
title: Submit a Signed Withdrawal Transaction for Broadcast
slug: /guides/staking-api/near/undelegate/submit-signed-withdraw-transaction
pagination_next: null
---

After signing the transaction, send a `PUT` request to `/api/v1/flows/[:flow_id]/next` (as always!) and the Staking API will broadcast the transaction to the NEAR network.

### Parameters

- `name` &mdash; `string` Name of the action to execute (required).
- `inputs` &mdash; `array`
  - `transaction_payload` &mdash; `string` Signed transaction payload from the previous step's response (required).

#### Sample cURL Request

```bash {5-8}
curl -X PUT 'https://near-slate.datahub.figment.io/api/v1/flows/96237daf-973a-456b-ae07-a5e16eeffbd1/next' \
-H 'Content-Type: application/json' \
-H 'Authorization: <api_key>' \
-d '{
  "name": "sign_withdraw_tx",
   "inputs": {
    "transaction_payload": ""
  }
}'
```

### Response

With the signed transaction provided, Staking API will broadcast it to the network. As the transaction progresses towards confirmation, the returned state will be either:

- `withdraw_tx_broadcasting` &mdash; Transaction has been broadcast and is waiting for confirmation.
- `withdrawn` &mdash; Transaction has been confirmed and the withdrawal is complete.

#### Sample Response

```json {3}
{
  "id": "96237daf-973a-456b-ae07-a5e16eeffbd1",
  "state": "withdraw_tx_broadcasting",
  "actions": [
    {
      "name": "wait",
      "estimated_state_change_at": "2022-09-28T16:48:37.242Z",
      "inputs": []
    }
  ],
  "data": {
    "delegator_address": "slate-demo.testnet",
    "delegator_pubkey": "ed25519:5QA46X6NkNmsFdu9xWVBaLNowh9gGeF1c5r9u6NcxaLY",
    "validator_address": "01node.pool.f863973.m0",
    "max_gas": null,
    "undelegate_amount": "25.0",
    "undelegate_maximum": null,
    "undelegate_transaction": {
      "raw": "",
      "signing_payload": "3ff41ac1a4b1f35e22412c3ebaa29b5e3075427929f111609d1cfc6706a27e2b",
      "signed": "",
      "hash": "5JeaBzEJguWfF35C8duE1vDRzP8rSQU4DM8T7stXLcEN",
      "status": "confirmed",
      "error": null,
      "signatures": []
    },
    "estimated_inactive_at": "2022-09-28T16:24:34.472Z",
    "withdraw_amount": "25.0",
    "withdraw_maximum": null,
    "withdraw_transaction": {
      "raw": "",
      "signing_payload": "b3988227217a53c7eaf6b904aa30c40fe218f81c6a9c6313baf22a305b5600b2",
      "signed": "",
      "hash": "D64riKQvR9ZjTgKG5rNBpQC8WDsxSNPxMQvLb8Ckyk81",
      "status": null,
      "error": null,
      "signatures": []
    }
  },
  "network_code": "near",
  "chain_code": "testnet",
  "created_at": "2022-09-26T16:15:50.379Z",
  "updated_at": "2022-09-28T16:47:36.737Z"
}
```
