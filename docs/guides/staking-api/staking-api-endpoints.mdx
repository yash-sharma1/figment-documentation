...
You will be provided with the HMAC `secret` when you first create a webhook. Remember to capture this data. Read more about authenticating webhook payloads in the <Link to="/guides/staking-api/staking-api-webhooks">Staking API Webhooks</Link> guide.
...
```bash {8} title="PUT /webhook_endpoints/[:webhook_endpoint_id]"
